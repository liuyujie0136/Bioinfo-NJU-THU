...
data <- read.table('Rplot-lulab-data/box_plots_mtcars.txt',header=T,sep="\t")
...
```{r}
df7 <- read.table('Rplot-lulab-data/box_plots_David_GO.txt',header=T,sep="\t")
df7 <- df7[1:10,]
...
```{r}
df2 <-read.table('Rplot-lulab-data/histogram_plots.txt',header=T,sep="\t")
```
...
ggplot(df, aes(x=cyl, y=mpg, color=cyl, fill=cyl, shape=cyl)) + 
...
```{r}
df3 <- read.table('Rplot-lulab-data/volcano_plots.txt',header=TRUE)
...
```{r}
df4 <- read.table('Rplot-lulab-data/manhattan_plots_gwasResults.txt',header=T,sep="\t")
```
...
```{r}
df5 <-read.table('Rplot-lulab-data/heatmaps.txt',header=T,sep="\t")
...
```{r}
df6 <- read.table('Rplot-lulab-data/ballon_plots_GO.txt',header=T,sep="\t")
```
...
or, in one column with reads count and percentage
